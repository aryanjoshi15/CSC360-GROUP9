...
- Branch per stage: `stage-2-layout`, `stage-3-fx-components`, and so on.
- Commit messages in the imperative: `add status bar to main layout`.
- No commits straight to `main` — open a pull request and have one teammate review.
- Class names in `PascalCase`, methods and fields in `camelCase`, packages lowercase.
